...
public class CellBroadcastAlertReminder extends Service {
    ...
    static boolean queueAlertReminder(final Context context, final int subId, final int duration, final Intent intent) {
        ...
        int interval = SubscriptionManager.getIntegerSubscriptionProperty(subId,
                SubscriptionManager.CB_ALERT_REMINDER_INTERVAL, 0, context);
        if (interval == 0) {
            loge("can't reminder");
            return false;
        }
        ...
        cancelAlertReminder();
        sPlayReminderIntent = PendingIntent.getService(context, 0, intent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        interval = interval * 60000 + duration;
        alarmManager.setRepeating(AlarmManager.RTC_WAKEUP,
                System.currentTimeMillis() + interval, interval, sPlayReminderIntent);
        return true;
    }
