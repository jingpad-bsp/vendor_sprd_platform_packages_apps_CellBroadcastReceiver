...
//        }
    ...
    public boolean loaderFromDb(boolean unexpected) {
        ...
        Cursor cursor = null;
        try {
            cursor = getContext().getContentResolver().query(Utils.mChannelUri,
    ...
    private boolean sendATCommand(){
        ...
        final CellBroadcastConfigManager cbcMgr = CellBroadcastConfigManager.getInstance(mSubId);
        for (int i = 0; i < countInsert; i++) {
            if((int)getInsertValues().get(i).get(ChannelTableDefine.ENABLE) == 1){
                ...
                cbcMgr.setGsmCellBroadcastChannel(true,
        ...
        while (iterator.hasNext()) {
            ...
            cbcMgr.setGsmCellBroadcastChannel((int)cv.get(ChannelTableDefine.ENABLE) == 1 ? true : false,
        ...
        for (int i = 0; i < countDelete; i++) {
            if (this.get((int)getDeleteValuesIndex().get(i)).getID() != -1) {
                ...
                cbcMgr.setGsmCellBroadcastChannel(false,
    ...
    public boolean SaveDataToDB() {
        synchronized(ChannelMgr.getInstance()){
            ...
            for (int i = 0; i < size; i++) {
                ...
                value.put(ChannelTableDefine.EDITABLE, get(i).getEditable());
                ...
                value.put(ChannelTableDefine.SOUND_URI, get(i).getSoundSetting().getSoundURI());
            ...
            //send AT command
            final boolean enableEmergencyAlerts = SubscriptionManager.getBooleanSubscriptionProperty(
                    mSubId, SubscriptionManager.CB_EMERGENCY_ALERT, true, mContext);
