...
public class SendTestMessages {
    ...
    private static final ComponentName mTargetComponentName = new ComponentName(
            "com.android.cellbroadcastreceiver",
            "com.android.cellbroadcastreceiver.PrivilegedCellBroadcastReceiver");
    ...
    private static SmsCbMessage createFromPdu(Context context, byte[] pdu, int serialNumber, int category, int subId) {
        byte[][] pdus = new byte[1][];
        pdus[0] = pdu;
        return createFromPdus(context, pdus, serialNumber, category, subId);
    }

    private static SmsCbMessage createFromPdus(Context context, byte[][] pdus, int serialNumber, int category, int subId) {
        try {
            for (byte[] pdu : pdus) {
                if (pdu.length <= 88) {
            ...
            return GsmSmsCbMessage.createSmsCbMessage(context, new SmsCbHeader(pdus[0]), sEmptyLocation, pdus, subId);
     ...
     */
    private static TextEncodingDetails codeCustomizeMessage(CharSequence msgBody,
                                                      boolean use7bitOnly) {
        CharSequence newMsgBody = null;
        Resources r = Resources.getSystem();
        if (getAndroidVersion() >= ANDROID_Q) {
    ...
    private static byte[] translate(final String message) {
        byte[] userData = {00};
        TextEncodingDetails ted = codeCustomizeMessage(message, false);
        try {
            userData = GsmAlphabet.stringToGsm7BitPacked(message, 0, false,
                    ted.languageTable, ted.languageShiftTable);
        } catch (EncodeException ex) {
            ex.printStackTrace();
        }
        byte[] header = {(byte)0xC0, (byte)0x00, (byte)0x00, (byte)0x01, (byte)0x40, (byte)0x11};
        int size = header.length + userData.length - 1;
        byte[] userData2 = new byte[size];

        for (int i = 0; i < size; i++) {
            userData2[i] = (byte)0x00;
        }
        for (int i = 0; i < header.length; i++) {
            userData2[i] = header[i];
        }
        for (int i = 1; i < userData.length; i++) {
            userData2[header.length + i - 1] = userData[i];
        }
        return userData2;
    }

    public static void testSendMessage7bit(Activity activity, int serialNumber, String message,
            int category, int subId) {
        Intent intent = new Intent(Intents.SMS_CB_RECEIVED_ACTION);
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        if (message == null || message.isEmpty()) {
            intent.putExtra("message", createFromPdu(activity, gsm7BitTest, serialNumber, category, subId));
        } else {
            intent.putExtra("message", createFromPdu(activity, translate(message), serialNumber, category, subId));
        }
    ...
    public static void testSendMessage7bitUmts(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsm7BitTestUmts, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessage7bitNoPadding(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsm7BitTestNoPadding, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessage7bitNoPaddingUmts(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsm7BitTestNoPaddingUmts, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessage7bitMultipageGsm(Activity activity, int serialNumber,
        ...
        intent.putExtra("message", createFromPdus(activity, pdus, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessage7bitMultipageUmts(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsm7BitTestMultipageUmts, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessage7bitWithLanguage(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity,gsm7BitTestWithLanguage, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessage7bitWithLanguageInBody(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsm7BitTestWithLanguageInBody, serialNumber,
                category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessage7bitWithLanguageInBodyUmts(Activity activity,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsm7BitTestWithLanguageInBodyUmts, serialNumber,
                category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessageUcs2(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsmUcs2Test, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessageUcs2Umts(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsmUcs2TestUmts, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessageUcs2MultipageUmts(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsmUcs2TestMultipageUmts, serialNumber, category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessageUcs2WithLanguageInBody(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsmUcs2TestWithLanguageInBody, serialNumber,
                category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendMessageUcs2WithLanguageUmts(Activity activity, int serialNumber,
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, gsmUcs2TestWithLanguageInBodyUmts, serialNumber,
                category, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL, Manifest.permission.RECEIVE_SMS,
                AppOpsManager.OP_RECEIVE_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendEtwsMessageNormal(Activity activity, int serialNumber, int subId) {
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, etwsMessageNormal, serialNumber, 0, subId));
    ...
    public static void testSendEtwsMessageCancel(Activity activity, int serialNumber, int subId) {
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, etwsMessageCancel, serialNumber, 0, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL,
                Manifest.permission.RECEIVE_EMERGENCY_BROADCAST,
                AppOpsManager.OP_RECEIVE_EMERGECY_SMS, null, null, Activity.RESULT_OK, null, null);
    }
    ...
    public static void testSendEtwsMessageTest(Activity activity, int serialNumber, int subId) {
        ...
        intent.putExtra(PhoneConstants.SUBSCRIPTION_KEY, subId);
        intent.setComponent(mTargetComponentName);
        intent.putExtra("message", createFromPdu(activity, etwsMessageTest, serialNumber, 0, subId));
        activity.sendOrderedBroadcastAsUser(intent, UserHandle.ALL,
                Manifest.permission.RECEIVE_EMERGENCY_BROADCAST,
                AppOpsManager.OP_RECEIVE_EMERGECY_SMS, null, null, Activity.RESULT_OK, null, null);
    }
